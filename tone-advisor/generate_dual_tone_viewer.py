...
function applyFilters() {
  var query = (document.getElementById('searchInputSidebar')?.value || '').toLowerCase();
  ...
  items.forEach(function(el) {
    ...
    var title = el.querySelector('.sidebar-title')?.textContent.toLowerCase() || '';
    var sub = el.querySelector('.sidebar-sub')?.textContent.toLowerCase() || '';
    var tags = el.querySelector('.sidebar-tags')?.textContent.toLowerCase() || '';
...
// Keyboard navigation between dual rigs
...
function toggleTheme() {
  ...
  localStorage.setItem('dtv-theme', next);
...
function toggleFilters() {
  ...
  localStorage.setItem('dtv-filters-collapsed', isCollapsed ? 'true' : 'false');
}

(function() {
  var savedTheme = localStorage.getItem('dtv-theme');
  var btn = document.getElementById('theme-toggle');
  if (savedTheme && btn) {
    document.documentElement.dataset.theme = savedTheme;
    btn.innerHTML = savedTheme === 'dark' ? '&#9788; Light' : '&#9790; Dark';
  }
  
  var collapsed = localStorage.getItem('dtv-filters-collapsed');
  if (collapsed === 'true') {
    var container = document.getElementById('filters-container');
    var icon = document.getElementById('filters-toggle-icon');
...
<script>(function(){{var t=localStorage.getItem('dtv-theme');if(t)document.documentElement.dataset.theme=t;}}());</script>
<style>
{CSS}
</style>
</head>
